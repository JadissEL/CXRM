...
export default function ClientDashboard() {
  ...
  // Handler for AI style recommendation
  const handleAIStyleUpload = async (file: File) => {
    setAiLoading(true);
    setAiError(null);
    setAiResults([]);
    ...
    try {
      const res = await axios.post("/api/ai/style-recommendation", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      setAiResults(res.data.recommendations);
    } catch (e: any) {
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <Tabs defaultValue="appointments" className="space-y-6">
          ...
          <TabsContent value="loyalty" className="space-y-6">
            {loyaltyPoints && (
              <>
                <Button className="mb-4" variant="outline" onClick={() => setShowAIModal(true)}>
                  <span role="img" aria-label="AI">🤖</span> Recommandation de coupe par IA
                </Button>
                {showAIModal && (
                  <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
                    <div className="bg-white rounded-xl p-8 max-w-lg w-full relative">
                      <button className="absolute top-2 right-2 text-gray-500" onClick={() => setShowAIModal(false)}>&times;</button>
                      <h2 className="text-xl font-bold mb-4">Recommandation de coupe par IA</h2>
                      <input type="file" accept="image/*" onChange={e => {
                        if (e.target.files && e.target.files[0]) handleAIStyleUpload(e.target.files[0]);
                      }} />
                      {aiLoading && <div className="mt-4">Analyse en cours...</div>}
                      {aiError && <div className="mt-4 text-red-600">{aiError}</div>}
                      {aiResults.length > 0 && (
                        <div className="mt-4 space-y-4">
                          {aiResults.map((style, idx) => (
                            <div key={idx} className="flex items-center gap-4 p-2 border rounded-lg">
                              <img src={style.image} alt={style.name} className="w-16 h-16 object-cover rounded" />
                              <div>
                ...
                <LoyaltyCard
                  points={loyaltyPoints.current_points}
                  onRedeem={() => {
                    // TODO: Open redeem modal or call API
                    alert("Fonctionnalité d'échange de points à venir");
                  }}
                  onSendGift={() => {
                    // TODO: Open send gift modal or call API
                    alert("Fonctionnalité d'envoi de carte cadeau à venir");
                  }}
                />
              </>
            )}
