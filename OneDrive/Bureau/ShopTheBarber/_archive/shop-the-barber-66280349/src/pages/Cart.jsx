...
export default function Cart() {
  ...
  React.useEffect(() => {
    ...
    if (savedCart) {
      try { setCartItems(JSON.parse(savedCart)); } catch (error) { console.error('Error loading cart:', error); }
    }
  }, []);
  ...
  const updateQuantity = (productId, newQuantity) => {
    ...
    const updated = cartItems.map(item => item.id === productId ? { ...item, quantity: newQuantity } : item);
    saveCart(updated);
  };
          ...
          <Card className="rounded-[12px] border-2 border-slate-200 p-12">
            ...
            <Link to={createPageUrl("Marketplace")}>
              <Button className="bg-[#D08B3D] hover:bg-[#D08B3D]/90 text-white px-8 py-6 text-lg rounded-[10px] min-h-[44px]">
                <Package className="w-5 h-5 mr-2" />Explorer la Marketplace
              </Button>
            </Link>
          </Card>
        </div>
    ...
    <div className="min-h-screen py-12 bg-[#F7F8FA]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          ...
          <p className="text-[#4B5563]">{cartItems.length} {cartItems.length > 1 ? 'articles' : 'article'}</p>
        </motion.div>
              ...
              <CardContent className="p-6">
                ...
                <div className="space-y-4 mb-6">
                  <div className="flex justify-between text-[#4B5563]">
                    <span>Sous-total</span><span className="font-semibold text-[#0B2545]">{getTotal().toFixed(2)}€</span>
                  </div>
                  <div className="flex justify-between text-[#4B5563]">
                  ...
                  <div className="flex justify-between text-xl font-bold text-[#0B2545]">
                    <span>Total</span><span className="text-[#D08B3D]">{getTotal().toFixed(2)}€</span>
                  </div>
                </div>
