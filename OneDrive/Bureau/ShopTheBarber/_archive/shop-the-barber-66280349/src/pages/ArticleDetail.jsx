...
export default function ArticleDetail() {
  ...
  const { data: article, isLoading } = useQuery({
    ...
    queryFn: () => base44.entities.Article.list().then(list => list.find(a => a.id === articleId)),
    enabled: !!articleId
  });

  const incrementViewMutation = useMutation({
    mutationFn: async () => { if (article) await base44.entities.Article.update(article.id, { views: (article.views || 0) + 1 }); },
