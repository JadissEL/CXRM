    ...
    <div className="min-h-screen py-12 bg-background-light dark:bg-background-dark font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12">
          {[
            { icon: Calendar, label: "RDV à venir", value: upcomingBookings.length, color: "text-primary" },
            { icon: ShoppingBag, label: "Commandes", value: orders.length, color: "text-emerald-600" },
            { icon: Star, label: "Visites", value: bookings.filter(b => b.status === 'completed').length, color: "text-amber-500" },
            { icon: TrendingUp, label: "Dépenses", value: `${bookings.reduce((sum, b) => sum + (b.total_price || 0), 0)}€`, color: "text-slate" }
          ].map((stat, index) => (
            <motion.div
              key={index}
              initial={{ opacity: 0, y: 20 }}
        ...
        <Tabs defaultValue="bookings" className="space-y-8">
          <TabsList className="bg-surface-light dark:bg-surface-dark p-1 rounded-xl shadow-sm border border-soft-gray dark:border-slate/20 inline-flex h-auto">
            <TabsTrigger
              value="bookings"
              className="rounded-lg px-6 py-3 data-[state=active]:bg-primary data-[state=active]:text-white data-[state=active]:shadow-md transition-all"
            >
