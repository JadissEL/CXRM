...
export default function Wishlist() {
  ...
  const { data: collections = [] } = useQuery({
    ...
    queryFn: async () => user ? await base44.entities.WishlistCollection.filter({ user_id: user.id }, 'order_index') : [],
    enabled: !!user
  });

  const { data: priceAlerts = [] } = useQuery({
    queryKey: ['price-alerts', user?.id],
    queryFn: async () => user ? await base44.entities.PriceAlert.filter({ user_id: user.id, is_active: true }) : [],
    enabled: !!user
  });
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#F7F8FA] py-12">
        <Card className="max-w-md w-full mx-4 rounded-[12px]">
          <CardContent className="p-12 text-center">
            ...
            <Button onClick={() => base44.auth.redirectToLogin(window.location.href)} className="bg-[#D08B3D] hover:bg-[#D08B3D]/90 text-white rounded-[10px] min-h-[44px]">Se connecter</Button>
          </CardContent>
        </Card>
      </div>
    );
