...
if (!JWT_SECRET) {
  if (process.env.NODE_ENV === "production") {
    ...
    console.warn("Warning: JWT_SECRET is not set. Using insecure fallback for development.");
  }
}
...
app.get("/api/barbers", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la récupération des barbiers" });
  }
});
...
app.get("/api/barbers/:id", async (req, res) => {
  try {
    ...
    const availability = await query(
      "SELECT * FROM barber_availability WHERE barber_id = ?",
      [id],
    );
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la récupération du barbier" });
  }
});
...
app.get("/api/appointments", authenticateToken, async (req, res) => {
  try {
    ...
    const appointmentsWithImageUrls = appointments.map((appointment) => ({
      ...appointment,
      reference_image: appointment.reference_image
        ? `http://localhost:3001${appointment.reference_image}`
        : null,
    }));
...
app.delete("/api/favorites/:barberId", authenticateToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la suppression des favoris" });
  }
});
...
app.get("/api/profile", authenticateToken, async (req, res) => {
  try {
    ...
    const user = await get(
      "SELECT id, email, first_name, last_name, phone, city, address, avatar_url, role FROM users WHERE id = ?",
      [userId],
    );
    ...
    if (user.role === "barber") {
      const barber = await get("SELECT * FROM barbers WHERE user_id = ?", [
        userId,
      ]);
      if (barber) {
...
app.post("/api/barber-profile", authenticateToken, async (req, res) => {
  try {
    ...
    if (user.role !== "barber") {
      return res
        .status(403)
        .json({ error: "Seuls les barbiers peuvent créer un profil barbier" });
    }

    // Check if barber profile already exists
    const existingBarber = await get(
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la création du profil barbier" });
  }
});
    ...
    if (!settings) {
      ...
      `,
        [
          ...
          JSON.stringify(defaultSettings.app),
        ],
      );
...
app.get("/api/barber-settings", authenticateToken, async (req, res) => {
  try {
    ...
    const settings = await get(
      "SELECT * FROM barber_settings WHERE user_id = ?",
      [userId],
    );
    ...
    if (!settings) {
      ...
      `,
        [
          userId,
          JSON.stringify(defaultSettings.business),
          JSON.stringify(defaultSettings.notifications),
        ],
      );
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la récupération des paramètres barbier" });
  }
});
...
app.put("/api/barber-settings", authenticateToken, async (req, res) => {
  try {
    ...
    if (barberProfile) {
      ...
      `,
        [
          ...
          barberProfile.acceptsHome ? 1 : 0,
          barberProfile.acceptsShop ? 1 : 0,
          userId,
        ],
      );
    }
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la mise à jour des paramètres barbier" });
  }
});
  ...
  try {
    ...
    if (!settings) {
      ...
      `,
        [
          userId,
          JSON.stringify(defaultSettings.system),
          JSON.stringify(defaultSettings.security),
          JSON.stringify(defaultSettings.moderation),
        ],
      );
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la récupération des paramètres admin" });
  }
});
...
app.put("/api/admin-settings", authenticateToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la mise à jour des paramètres admin" });
  }
});
  ...
  try {
    ...
    if (user.role === "admin") {
      ...
      const totalRevenue = await get(
        'SELECT SUM(total_price) as total FROM appointments WHERE status = "completed"',
      );
      ...
      // Barber analytics
      const barber = await get("SELECT id FROM barbers WHERE user_id = ?", [
        userId,
      ]);
      if (!barber) {
      ...
      const completedAppointments = await get(
        'SELECT COUNT(*) as count FROM appointments WHERE barber_id = ? AND status = "completed"',
        [barber.id],
      );
      const totalRevenue = await get(
        'SELECT SUM(total_price) as total FROM appointments WHERE barber_id = ? AND status = "completed"',
        [barber.id],
      );
      const avgRating = await get(
        "SELECT AVG(rating) as avg FROM reviews WHERE barber_id = ?",
        [barber.id],
      );
      ...
      // Client analytics
      const totalBookings = await get(
        "SELECT COUNT(*) as count FROM appointments WHERE client_id = ?",
        [userId],
      );
      const completedBookings = await get(
        'SELECT COUNT(*) as count FROM appointments WHERE client_id = ? AND status = "completed"',
        [userId],
      );
      const totalSpent = await get(
        'SELECT SUM(total_price) as total FROM appointments WHERE client_id = ? AND status = "completed"',
        [userId],
      );
      const favoriteBarbers = await get(
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la récupération des analytics" });
  }
});

// Client analytics for charts
app.get("/api/analytics/client", authenticateToken, async (req, res) => {
  try {
    const userId = req.user.userId;

    // Get monthly bookings data for client
    const monthlyBookings = await query(
      `
      SELECT strftime('%Y-%m', appointment_date) as month, COUNT(*) as count
      FROM appointments
      WHERE client_id = ? AND appointment_date >= date('now', '-6 months')
      GROUP BY month
      ORDER BY month
    `,
      [userId],
    ...
    // Get status distribution for client
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la récupération des analytics client" });
  }
});
...
app.get("/api/analytics/revenue", authenticateToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la récupération des données de revenus" });
  }
});
...
app.get("/api/analytics/appointments", authenticateToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        error: "Erreur lors de la récupération des données de rendez-vous",
      });
  }
});
  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        req.user.userId !== parseInt(barberId)
      ) {
        return res.status(403).json({ error: "Accès non autorisé" });
      }
...
app.post("/api/barber-services", authenticateToken, async (req, res) => {
  try {
    const { barberId, serviceId, price, customName, customDescription } =
      req.body;
    ...
    if (existingService) {
      return res
        .status(400)
        .json({ error: "Ce service existe déjà pour ce barbier" });
    }
...
app.post("/api/barber-videos", authenticateToken, async (req, res) => {
  try {
    const { barberId, title, description, videoUrl, thumbnailUrl, duration } =
      req.body;

    // Vérifier que l'utilisateur est le propriétaire du profil barbier
    const barber = await get(
      "SELECT id FROM barbers WHERE id = ? AND user_id = ?",
    ...
    const videoCount = await get(
      "SELECT COUNT(*) as count FROM barber_videos WHERE barber_id = ? AND is_active = 1",
      [barberId],
    );
    if (videoCount.count >= 3) {
      return res
        .status(400)
        .json({ error: "Vous ne pouvez pas avoir plus de 3 vidéos actives" });
    }

    // Obtenir le prochain order_index
    const maxOrder = await get(
      "SELECT MAX(order_index) as max_order FROM barber_videos WHERE barber_id = ?",
      [barberId],
    );
...
app.put("/api/barber-videos/:id", authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    const { title, description, videoUrl, thumbnailUrl, duration, orderIndex } =
      req.body;
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la mise à jour de la vidéo" });
  }
});
...
app.delete("/api/barber-videos/:id", authenticateToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la suppression de la vidéo" });
  }
});
...
app.get("/api/blog/articles", async (req, res) => {
  try {
    ...
    sql +=
      " GROUP BY ba.id ORDER BY ba.published_at DESC, ba.created_at DESC LIMIT ? OFFSET ?";
...
app.get("/api/blog/articles/:id", async (req, res) => {
  try {
    ...
    await run(
      "UPDATE blog_articles SET view_count = view_count + 1 WHERE id = ?",
      [id],
    );
  ...
  try {
    ...
    if (!["barber", "admin"].includes(authorType)) {
      return res
        ...
        .json({
          error:
            "Seuls les barbiers et administrateurs peuvent créer des articles",
        });
    }
    ...
    if (categories && categories.length > 0) {
      for (const categoryName of categories) {
        ...
        let category = await get(
          "SELECT id FROM article_categories WHERE name = ?",
          [categoryName],
        );
        if (!category) {
...
app.put("/api/blog/articles/:id", authenticateToken, async (req, res) => {
  try {
    ...
    const article = await get(
      "SELECT author_id, author_type FROM blog_articles WHERE id = ?",
      [id],
    );
    if (!article) {
      return res.status(404).json({ error: "Article non trouvé" });
    ...
    if (categories) {
      ...
      await run("DELETE FROM article_category_relations WHERE article_id = ?", [
        id,
      ]);
...
app.delete("/api/blog/articles/:id", authenticateToken, async (req, res) => {
  try {
    ...
    // Vérifier que l'utilisateur est l'auteur ou un admin
    const article = await get(
      "SELECT author_id FROM blog_articles WHERE id = ?",
      [id],
    );
    if (!article) {
      return res.status(404).json({ error: "Article non trouvé" });
...
app.post("/api/blog/articles/:id/like", authenticateToken, async (req, res) => {
  try {
    ...
    const article = await get("SELECT id FROM blog_articles WHERE id = ?", [
      id,
    ]);
    if (!article) {
      return res.status(404).json({ error: "Article non trouvé" });
    ...
    // Ajouter le like
    await run("INSERT INTO article_likes (article_id, user_id) VALUES (?, ?)", [
      id,
      userId,
    ]);
    ...
    await run(
      "UPDATE blog_articles SET likes_count = likes_count + 1 WHERE id = ?",
      [id],
    );
...
app.patch(
  ...
  async (req, res) => {
    try {
      ...
      await run("UPDATE barbers SET is_verified = ? WHERE id = ?", [
        is_verified,
        id,
      ]);
      ...
      res
        .status(500)
        .json({ error: "Erreur lors de la mise à jour de la vérification" });
    }
  },
);

app.patch(
  "/api/admin/barbers/:id/toggle-status",
  authenticateToken,
  async (req, res) => {
    try {
      // Vérifier que l'utilisateur est admin
      ...
      await run("UPDATE barbers SET is_active = ? WHERE id = ?", [
        is_active,
        id,
      ]);
...
app.get(
  ...
  async (req, res) => {
    try {
      // Vérifier que l'utilisateur est admin
      const user = await get("SELECT role FROM users WHERE id = ?", [
        req.user.userId,
      ]);
      if (user.role !== "admin") {
        return res.status(403).json({ error: "Accès non autorisé" });
      ...
      res
        .status(500)
        .json({ error: "Erreur lors de la récupération du contenu à modérer" });
    }
  },
);
...
app.put(
  "/api/admin/moderation/articles/:id",
  authenticateToken,
  async (req, res) => {
    try {
      // Vérifier que l'utilisateur est admin
      ...
      const article = await get("SELECT * FROM blog_articles WHERE id = ?", [
        id,
      ]);
      if (!article) {
        return res.status(404).json({ error: "Article non trouvé" });
      ...
      if (action !== "delete" && article.author_id) {
        const notificationTitle =
          action === "approve" ? "Article approuvé" : "Article rejeté";
      ...
      res
        .status(500)
        .json({ error: "Erreur lors de la modération de l'article" });
    }
  },
);
...
app.put(
  "/api/admin/moderation/videos/:id",
  authenticateToken,
  async (req, res) => {
    try {
      // Vérifier que l'utilisateur est admin
      ...
      const video = await get(
        "SELECT bv.*, b.user_id FROM barber_videos bv JOIN barbers b ON bv.barber_id = b.id WHERE bv.id = ?",
        [id],
      );
      if (!video) {
        return res.status(404).json({ error: "Vidéo non trouvée" });
      ...
      switch (action) {
        case "approve":
          ...
          await run("UPDATE barber_videos SET is_active = 1 WHERE id = ?", [
            id,
          ]);
          break;
        ...
        case "reject":
          ...
          await run("UPDATE barber_videos SET is_active = 0 WHERE id = ?", [
            id,
          ]);
          break;
      ...
      if (action !== "delete" && video.user_id) {
        const notificationTitle =
          action === "approve" ? "Vidéo approuvée" : "Vidéo rejetée";
        const notificationMessage =
          action === "approve"
      ...
      res
        .status(500)
        .json({ error: "Erreur lors de la modération de la vidéo" });
    }
  },
);
...
// Accepter
app.put(
  "/api/admin/blog-articles/:id/approve",
  authenticateToken,
  requireAdmin,
  async (req, res) => {
    try {
      await run(
        "UPDATE blog_articles SET status = 'published', published_at = CURRENT_TIMESTAMP WHERE id = ?",
        [req.params.id],
      );
...
// Refuser
app.put(
  "/api/admin/blog-articles/:id/reject",
  authenticateToken,
  requireAdmin,
  async (req, res) => {
    try {
      await run("UPDATE blog_articles SET status = 'rejected' WHERE id = ?", [
        req.params.id,
      ]);
    ...
    try {
      ...
      if (status) {
        if (status === "pending") {
          ...
          sql +=
            " WHERE is_active = 0 AND (status = 'rejected' OR status IS NULL)";
        }
      }
      sql += " ORDER BY created_at DESC";
...
app.get(
  "/api/admin/barber-videos/:id",
  authenticateToken,
  requireAdmin,
  async (req, res) => {
    try {
      const video = await get("SELECT * FROM barber_videos WHERE id = ?", [
        req.params.id,
      ]);
...
app.put(
  "/api/admin/barber-videos/:id/approve",
  authenticateToken,
  requireAdmin,
  async (req, res) => {
    try {
...
app.put(
  "/api/admin/barber-videos/:id/reject",
  authenticateToken,
  requireAdmin,
  async (req, res) => {
    try {
      await run(
        "UPDATE barber_videos SET is_active = 0, status = 'rejected' WHERE id = ?",
        [req.params.id],
      );
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Erreur lors de la récupération de l'avis" });
    }
  },
);
app.put(
  ...
  async (req, res) => {
    try {
      await run("UPDATE reviews SET status = 'approved' WHERE id = ?", [
        req.params.id,
      ]);
...
app.put(
  ...
  async (req, res) => {
    try {
      await run("UPDATE reviews SET status = 'rejected' WHERE id = ?", [
        req.params.id,
      ]);
