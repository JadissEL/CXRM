...
export default function Index() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-moroccan-charcoal via-moroccan-darkgrey to-moroccan-green/60 text-foreground font-sans flex flex-col">
      {/* Glassy Navbar */}
      <header className="sticky top-0 z-50 w-full backdrop-blur bg-white/70 dark:bg-moroccan-charcoal/80 shadow-lg rounded-b-2xl border-b border-moroccan-gold/10 transition-all duration-300">
        <div className="container mx-auto flex h-16 items-center justify-between px-4">
          <Link to="/" className="flex items-center space-x-2">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-moroccan-gold to-moroccan-green text-white shadow-md animate-pulse">
          ...
          <div className="flex items-center space-x-2 md:space-x-4">
            <Button variant="ghost" size="icon" className="text-moroccan-gold hover:bg-moroccan-gold/10 animate-bounce">
      ...
      {/* Hero Section */}
      <section className="relative flex-1 flex flex-col justify-center items-center text-center py-24 px-4 overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute inset-0 bg-moroccan-gradient-primary opacity-60 animate-gradient-x" />
          <MoroccanPattern className="absolute inset-0 w-full h-full opacity-10" />
        </div>
        <div className="relative z-10 max-w-3xl mx-auto">
          <h1 className="text-5xl md:text-6xl font-extrabold bg-gradient-to-br from-moroccan-gold via-moroccan-green to-moroccan-royal-blue bg-clip-text text-transparent drop-shadow-lg mb-6 animate-fade-in">
            Réservez votre expérience barbershop premium
          </h1>
          <p className="text-xl md:text-2xl text-moroccan-sand mb-10 animate-fade-in delay-100">
            Trouvez, réservez et vivez le meilleur du grooming marocain, partout, à tout moment.
          </p>
          <Button
            size="xl"
            className="bg-gradient-to-r from-moroccan-gold to-moroccan-green text-white shadow-xl rounded-full px-10 py-5 text-2xl font-bold animate-bounce hover:scale-105 hover:shadow-2xl transition-all duration-300"
        ...
        {/* Glassy Card Overlay */}
        <div className="relative z-10 mt-16 grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {features.map((feature, idx) => (
            <Card key={idx} className="bg-white/70 dark:bg-moroccan-darkgrey/80 backdrop-blur-md border-0 shadow-2xl rounded-2xl p-8 flex flex-col items-center hover:scale-105 hover:shadow-amber-500/30 transition-all duration-300 animate-slide-up">
              <div className="mb-4">
                <feature.icon className="h-10 w-10 text-moroccan-gold animate-gold-glow" />
              </div>
              <h3 className="text-xl font-bold mb-2 text-moroccan-charcoal dark:text-moroccan-gold">{feature.title}</h3>
              <p className="text-moroccan-sand mb-2">{feature.description}</p>
              <img src={feature.image} alt={feature.title} className="rounded-xl shadow-lg w-full h-32 object-cover object-center moroccan-image-glow" />
            </Card>
          ))}
        </div>
      </section>
      ...
      {/* Glassy Footer */}
      <footer className="w-full mt-auto py-8 bg-white/70 dark:bg-moroccan-charcoal/80 backdrop-blur-md shadow-inner rounded-t-2xl border-t border-moroccan-gold/10 flex flex-col items-center space-y-2 animate-fade-in">
        <div className="flex space-x-4 mb-2">
          <a href="https://instagram.com" target="_blank" rel="noopener" className="text-moroccan-gold hover:text-moroccan-green transition-colors duration-200">
            <Sparkles className="h-6 w-6" />
          </a>
          <a href="https://facebook.com" target="_blank" rel="noopener" className="text-moroccan-gold hover:text-moroccan-green transition-colors duration-200">
            <User className="h-6 w-6" />
          </a>
          <a href="https://discord.com" target="_blank" rel="noopener" className="text-moroccan-gold hover:text-moroccan-green transition-colors duration-200">
            <Scissors className="h-6 w-6" />
          </a>
        </div>
        <span className="text-moroccan-sand text-sm">© {new Date().getFullYear()} ShopTheBarber. Design premium inspiré par le Maroc.</span>
      </footer>
