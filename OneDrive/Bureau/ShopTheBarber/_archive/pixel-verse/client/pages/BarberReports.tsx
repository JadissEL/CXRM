    ...
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* No Data Message */}
          {(!overviewData?.monthlyRevenue?.length && !overviewData?.topServices?.length && revenueData.length === 0 && !performanceData?.monthlyPerformance?.length) ? (
            <div className="col-span-2 flex flex-col items-center justify-center py-16">
              <div className="text-6xl mb-4">📉</div>
              <h3 className="text-2xl font-bold text-moroccan-gold mb-2">Aucune donnée à afficher</h3>
              <p className="text-lg text-gray-300 mb-2 max-w-xl text-center">
                Vous n'avez pas encore d'activité enregistrée. Dès que vous commencerez à prendre des rendez-vous et à générer des revenus, vos statistiques et graphiques apparaîtront ici.
              </p>
              <div className="mt-4 p-4 bg-moroccan-darkgrey/80 rounded-lg border border-moroccan-gold/30 text-moroccan-gold text-sm max-w-lg text-center">
                Astuce : <b>Invitez vos clients à réserver via la plateforme</b> pour commencer à suivre vos performances, revenus et services les plus populaires !
              </div>
            </div>
          ) : (
            <>
