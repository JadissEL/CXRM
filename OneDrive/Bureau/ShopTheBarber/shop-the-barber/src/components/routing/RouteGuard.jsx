...
export default function RouteGuard() {
  ...
  const { data: isManager, isLoading: isManagerLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (members.some((m) => ['owner', 'manager'].includes(m.role))) return true;
      const owned = await sovereign.entities.Shop.filter({ owner_id: user.id });
  ...
  useEffect(() => {
    ...
    const onManagerPage = managerPages.some((p) => path === p || path.startsWith(`${p}/`));
    if (onManagerPage && isManager === false) {
