...
export function useManagedShop() {
  ...
  const { data: ownedShop, isLoading: ownedShopLoading } = useQuery({
    queryKey: ['my-owned-shop', user?.id],
    queryFn: async () => {
  ...
  return {
    ...
    isLoading:
      isLoadingAuth ||
      (!!user &&
        (barberLoading || ownedShopLoading || memberLoading || staffMemberLoading || shopLoading)),
  };
}
